...
const updateCourse = async (req, res) => {
    try {
        ...
        const course = await Course.findByIdAndUpdate(req.params.id, validatedFields, { new: true });
        if (!course) {
            return res.status(404).json({ errorMessage: 'Course not found' });
        }
        res.status(200).json(course);
...
const addTeacher = async (req, res) => {
    try
    {
        ...
        if ( !email || !password || !CNIC || !name || !contactNumber || !address) {
            throw new Error('Please provide values for email, password, CNIC, name, contactNumber, and address.');
        }
        const teacher = new Teacher({email, password,CNIC,name,contactNumber,address});
...
const updateTeacher = async (req, res) => {
    try {
        ...
        teacher.contactNumber=contactNumber?contactNumber:teacher.contactNumber;
